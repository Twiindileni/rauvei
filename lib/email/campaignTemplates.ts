...
export function renderCampaignHtml(input: TemplateParams): string {
  ...
  const couponBadge =
    input.kind === "coupon" && input.couponCode
      ? `<div style="margin:18px 0;padding:14px 18px;border:1px dashed #c8956c;background:#fff8f1;font-size:20px;letter-spacing:.1em;font-weight:700;color:#8a5b2e;text-align:center">${esc(input.couponCode)}</div>`
      : "";

  const invoiceMeta =
    input.kind === "invoice"
      ? `
      <div style="margin:12px 0 8px;padding:12px 14px;background:#fdf8f2;border:1px solid #efe4d7">
        <p style="margin:0 0 6px;color:#6f5338;font-size:13px">Invoice reference: <strong>#${esc(input.orderId ?? "—")}</strong></p>
        <p style="margin:0 0 6px;color:#6f5338;font-size:13px">Order date: <strong>${esc(input.orderDate ?? "—")}</strong></p>
        <p style="margin:0;color:#6f5338;font-size:13px">Total amount: <strong>N$${Number(input.orderTotal ?? 0).toFixed(2)}</strong></p>
      </div>
  ...
  const cta =
    input.ctaLabel && input.ctaUrl
      ? `<a href="${esc(input.ctaUrl)}" style="display:inline-block;margin-top:18px;background:#c8956c;color:#fff;text-decoration:none;padding:12px 18px;font-weight:600;letter-spacing:.03em">${esc(input.ctaLabel)}</a>`
      : "";
